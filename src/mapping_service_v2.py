...
class MappingServiceV2:
    ...
    def _is_option_like_field(cls, field: pd.Series | dict[str, Any]) -> bool:
        ...
        return (
            has_options or
            bool(reference_type) or
            cls._is_choice_value_model(value_model) or
            field_type in {"ReferenceField", "OptionChoiceField"}
        )
        ...
        for df_col in sorted(upload_columns):
            ...
            if mapped_schema_field and schema_exists:
                ...
                if not matched.empty:
                    ...
                    row["is_option_field"] = bool(m.get("is_option_like", False))
                    row["option_source_kind"] = m.get("option_source_kind")
