...
- 현재 아이템 상세를 열면 첨부 목록의 raster 이미지 MIME을 자동으로 메모리에 받아 첨부 영역에 표시합니다.
  ...
  흐름을 유지합니다.
- `상세 크게 보기` 창은 아이템 개요·전체 필드·관계·참조·변경 이력·원본 JSON과 첨부 이미지 탭을 제공합니다.
  관계와 이력은 현재 아이템에서 탭을 열 때만 조회하며, 관련 아이템은 같은 창의 뒤로·앞으로 기록 안에서
  이동합니다. Baseline 상세에서는 현재 관계·이력을 대신 불러오지 않습니다. 첨부 이미지
  탭은 여러 이미지를 선택할 수 있고 축소·100%·화면 맞춤·확대 버튼과 Ctrl+마우스 휠을 지원합니다.
- 현재 아이템의 댓글 탭은 처음 열 때 목록을 조회하고 `replyTo`를 대화 스레드로 표시합니다. Wiki 댓글은
  본문과 같은 sanitizer·fallback을 사용하며, 댓글 이미지는 인증된 content service와 동일한 리소스
  한도를 적용합니다. 비이미지 첨부는 명시적 저장만 허용하고 Baseline 댓글은 조회하지 않습니다.
