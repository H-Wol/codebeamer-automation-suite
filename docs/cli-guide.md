...
- Excel 입력을 `reader -> raw dataframe -> hierarchy processor` 구조로 분리했습니다.
...
- `TrackerItemChoiceField` 는 입력값의 tracker item ID를 파싱해 `TrackerItemReference`로 변환합니다. 이름·summary query lookup은 비활성화되어 있습니다.
...
- GUI와 별도 설정도 동일하게 ID 직접 파싱만 사용합니다.
