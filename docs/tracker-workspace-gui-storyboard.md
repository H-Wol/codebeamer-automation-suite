...
| Phase 1A | 필요한 아이템을 계층 또는 ID로 빠르게 찾고 확인 | 프로젝트·트래커 선택, 최상위·하위 반복 탐색, ID 직접 접근, 전체 필드·설명·custom field 상세, 부모·자식 요약, 원본 JSON | 모든 쓰기 |
...
| 부분 구현 | 아이템 맥락과 문서 내용 확인 | 선택형 Wiki table 서버 렌더링, 읽기 전용 아이템 상세 창, 현재 관계·참조·변경 이력·댓글 스레드와 댓글 첨부, 관련 아이템 같은 창 이동, 첨부 이미지 자동 표시와 확대·축소, ID 기반 첨부 메타데이터와 저장 | 댓글·첨부·관계 변경, Baseline 과거 관계·이력·댓글·첨부 |
...
```text
┌──────────────────────────────────────────────────────────────────────────────┐
│ 1042 · Brake requirement    Open   [웹에서 열기] [상태 필드 변경] [편집]  │
├──────────────────────────────────────────────────────────────────────────────┤
...
- 현재 `상태 필드 변경`은 `PUT /fields` 부분 갱신임을 명시합니다. 실제 `상태 전환`은 Phase 2에서 workflow endpoint 계약 확인 후 별도로 활성화합니다.
...
│ Status         Open     [상태 필드 변경] · workflow transition 아님          │
│                                                                              │
│ 일반 필드                                                                   │
...
| Status 필드 변경 | 구현 | `PUT /fields` 부분 갱신이며 workflow transition으로 표시하지 않음 |
| 실제 상태 전환 | 미구현 TODO | 현재 상태의 transition 조회·실행 방식 확인 |
...
| 상세 조회 1차 범위 | 전체 필드·설명·custom field, 부모·자식 요약, 원본 JSON | 현재 item 첨부·관계·이력·댓글 읽기와 선택 저장 부분 구현, Baseline 과거 문맥은 후속 |
| 광범위 편집 이후 순서 | Status 필드 변경, 단건 아이템 생성 | 구현 완료, 실제 workflow transition과 Baseline 과거 문맥은 후속 |
