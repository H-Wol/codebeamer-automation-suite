...
| 조건 검색 | `간편`, `조건 조합`, `CbQL` 세 모드를 분리해 제공 |
| 조건 논리식 | 그룹 내부 AND, 그룹 사이 OR의 1단계 조합 |
...
- 검색 UI는 `간편`, `조건 조합`, `CbQL` 세 모드로 나누고 활성 모드 하나만 실행합니다.
- `간편` 모드는 ID/요약, 상태, 담당자 등 자주 사용하는 필드를 제공합니다.
- `조건 조합` 모드는 현재 tracker schema에서 필드를 선택하고 field type에 맞는 연산자와 값 입력기를 제공합니다.
- 한 그룹 안의 조건은 AND로 결합하고, 여러 그룹은 OR로 결합합니다.
...
- `TrackerQuery`: 필수 tracker ID, search mode, 간편 필터·조건 조합·CbQL 중 하나, page, page size, sort, baseline
...
- `간편`, `조건 조합`, `CbQL` 중 활성 검색 모드 하나만 서버 query로 변환합니다.
- 조건 조합의 field, operator, value 입력기가 현재 tracker schema와 일치합니다.
- 조건 그룹 내부는 AND, 그룹 사이는 OR로 변환되고 빈 조건·그룹은 실행되지 않습니다.
...
| 검색 모드 | `간편 / 조건 조합 / CbQL` 분리 | 사용자 결정 완료, 활성 모드 하나만 실행 |
| 조건 논리식 | 그룹 내부 AND, 그룹 사이 OR의 1단계 구조 | 사용자 결정 완료, 복잡한 중첩은 CbQL 사용 |
