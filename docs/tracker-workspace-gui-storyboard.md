...
| 부분 구현 | 아이템 맥락과 문서 내용 확인 | 선택형 Wiki table 서버 렌더링, 읽기 전용 아이템 상세 창, 현재 댓글 스레드와 댓글 첨부, 첨부 이미지 자동 표시와 확대·축소, ID 기반 첨부 메타데이터와 저장 | 댓글·첨부·관계 변경, Baseline 과거 댓글·첨부 |
...
| 상세 조회 1차 범위 | 전체 필드·설명·custom field, 부모·자식 요약, 원본 JSON | 현재 item 첨부와 댓글 읽기·저장 부분 구현, 관계·이력과 Baseline 과거 문맥은 후속 |
