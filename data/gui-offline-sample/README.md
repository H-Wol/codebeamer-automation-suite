...
5. 파일 단계에서 `SWE6_BRAKE_R24_TC_001.xlsx`, `SWE6_MOTOR_R24_TC_002.xlsx`를 함께 선택합니다.
...
8. `Related Requirement`는 테스트 모드에서 query lookup이 아니라 기본 regex ID 추출 방식으로 두는 편이 맞습니다.
   - 샘플 값은 `[REQ:20263671]` 같은 형식이라 기본 regex로 바로 인식됩니다.
...
- 샘플 파일명은 `SWE6_<SUBSYSTEM>_R24_TC_<번호>.xlsx` 규칙이라 미리보기 검증에도 적합합니다.
