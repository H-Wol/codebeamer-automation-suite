...
- 기본 계층 탭에서 현재 상태 또는 Baseline을 선택하고, Baseline 전체 query의 `parent`·`children`·`ordinal`만으로 과거 계층과 읽기 전용 상세 조회
...
- 트래커 작업공간
  ...
  - 명시적 Wiki 형식 설명·필드는 온라인 `wiki2html` 서버 렌더링을 우선 사용하고 원문 전환과 안전한 로컬 단순 table fallback 제공
  - `TableField` 행·열 전용 보기와 Wiki 셀 최대 4개 동시 렌더링, 같은 서버의 인증된 첨부 이미지만 10MB/개·50MB/아이템 한도로 표시
  - 현재 아이템 첨부 메타데이터 목록과 사용자 선택 파일 저장; Baseline 과거 첨부는 서버 revision 계약 확인 전 차단
...
- Baseline 단일 계층은 기본 계층 탭의 `조회 기준`에서 Baseline을 고른 뒤 명시적으로 불러옵니다. 과거 시점 query를 모든 페이지에서 수집해 메모리 트리로 구성하며 현재 root/children API를 섞지 않습니다. Baseline 상세는 같은 `baselineId`로 조회하고 생성·수정·삭제·상태 전환·일괄 수정은 차단합니다.
