...
- 트래커 작업공간
  ...
  - 현재 계층과 신규·삭제·변경·동일 전체 결과를 함께 제공하고 선택 아이템 상세는 조회 캐시에서 표시
...
- Baseline 비교는 사용자가 전체 비교 실행을 확인한 경우에만 트래커 전체를 query로 조회해 결과를 캐시하고, 현재 계층·신규·삭제·변경·동일 결과·상세 비교와 2단 헤더 Excel 내보내기가 같은 데이터를 사용합니다. 셀 길이 또는 줄바꿈 한도를 넘는 값은 `긴 값 전체보기` 시트에 분할하므로 전체 내보내기가 중단되거나 원문이 잘리지 않습니다.
