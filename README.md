...
- Baseline 비교는 트래커 전체를 query로 한 번씩 조회해 결과를 캐시하고, 현재 계층·전체 결과·상세 비교와 2단 헤더 Excel 내보내기가 같은 데이터를 사용합니다. 셀 길이 또는 줄바꿈 한도를 넘는 값은 `긴 값 전체보기` 시트에 분할하므로 전체 내보내기가 중단되거나 원문이 잘리지 않습니다.
