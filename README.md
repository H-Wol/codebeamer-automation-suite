...
- `UserChoiceField`, `MemberField`, `UserReference` 필드에 대해 사용자 ID 기반 lookup 후 reference로 변환
...
7. 사용자 선택 필드는 사용자 ID로 `GET /v3/users/{userId}` 조회 후 reference로 변환하고 결과를 캐시에 저장합니다.
8. `TrackerItemChoiceField` 와 builtin `subjects` 는 입력값에서 tracker item ID를 파싱해 `TrackerItemReference`로 변환합니다.
9. row별 payload를 먼저 cache하고 preview와 upload가 같은 payload를 재사용합니다.
10. 업로드 시점에는 parentItemId만 `created_map[parent_row_id]` 기준으로 결정합니다.
11. 실행 결과와 중간 dataframe, schema, payload cache, 검증 결과를 `output/`에 저장할 수 있습니다.
...
- 사용자 관련 필드는 이름/이메일이 아니라 사용자 ID 기준으로만 lookup 합니다.
- `TrackerItemChoiceField` 와 builtin `subjects` 는 각 값에서 `[]` 안 첫 번째 정수를 우선 사용해 `TrackerItemReference`를 만듭니다.
