...
- 현재 tracker 범위를 강제하는 CbQL, 서버 pagination 메타데이터, 계층·상세 정규화 조회 서비스
- 프로젝트·트래커 선택, 지연 로딩 확장형 트리, tracker 범위 검색과 ID 바로 열기를 제공하는 작업공간
- schema 기반 선택 필드 부분 수정, 단건 상태 전환, version 충돌 확인과 ID 재입력 삭제
...
- `트래커 작업공간`에 프로젝트·트래커 선택, 확장형 계층, tracker 범위 검색, ID 직접 접근과 상세 화면을 연결했습니다.
- 상세 `수정` 탭에 schema 기반 광범위 필드 편집, 단건 상태 전환과 안전한 삭제를 연결했습니다.
- `실행 기록`은 단계별 구현 범위를 과장하지 않도록 현재 상태를 안내하는 화면으로 유지합니다.
