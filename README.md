...
- 간편 검색과 그룹 내부 AND·그룹 간 OR 방식의 다중 조건 검색, 조건별 제외 연산자 지원
- 전용 Baseline 비교 탭에서 현재 트래커 계층을 탐색하고, 선택한 단일 아이템의 표준·메타·사용자 정의 필드를 `기준`과 `비교` 시점으로 상세 비교하며 탭 이동 중 선택과 결과 유지
