...
`cli_main_v2.py`에는 다음 보강이 반영되어 있습니다.
...
- `src/mapping_service_v2.py`: 개선된 스키마/옵션 매핑 서비스
- `src/models.py`: payload 및 상태 도메인 모델
- `src/wizard.py`: 레거시 오케스트레이션 계층
- `src/wizard_v2.py`: 개선된 오케스트레이션 계층
...
- 현재 기준으로는 `cli_main_v2.py` 사용을 권장합니다.
