...
- 트래커 작업공간 기반 서비스
  ...
  - 현재 tracker 범위를 강제한 간편·조건 조합·CbQL 변환
  - 페이지 메타데이터, 전체 필드·custom field 상세와 parent 경로 정규화
  - 현재 브랜치에서는 서비스와 fixture까지 구현했으며 실제 작업공간 화면 연결은 다음 단계
- 배치 설정 화면
...
- `트래커 작업공간`과 `실행 기록`은 단계별 구현 범위를 과장하지 않도록 현재 상태를 안내하는 화면으로 유지합니다.
