...
- Baseline 단일 계층은 기본 계층 탭의 `조회 기준`에서 Baseline을 고른 뒤 명시적으로 불러옵니다. 과거 시점 query를 모든 페이지에서 수집해 메모리 트리로 구성하며 현재 root/children API를 섞지 않습니다. 조회가 끝나면 캐시된 계층과 tracker schema를 이용해 필드를 선택하고 Excel로 내보낼 수 있으며, 내보내기 과정에서 아이템별 API를 다시 호출하지 않습니다. Baseline 상세는 같은 `baselineId`로 조회하고 생성·수정·삭제·상태 전환·일괄 수정은 차단합니다.
